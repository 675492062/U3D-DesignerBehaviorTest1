...
public class ClickSelectTeamImageCallback : MonoBehaviour {
    ...
    void OnClick()
    {
        ...
        {
            TeamHeadWindow.gameObject.SetActive(false);
        }
        if (!GeneralWindow.gameObject.activeSelf)
        {
            GeneralWindow.gameObject.SetActive(true);
        }
    ...
    {
        GameObject instance = Instantiate(Resources.Load("Prefab/Player/Hero", typeof(GameObject))) as GameObject;
        ...
        ClickGeneralCallback s = instance.GetComponent<ClickGeneralCallback>();
        s.label.text = text.ToString();
        s.sprite.name = spritName;
        MonoInstancePool.getInstance<UserData>().TeamHeroObject.Add(instance);
    }
}
